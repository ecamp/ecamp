 ...
 */
    ...
    // Zeitfenster-Veränderung vornehmen
      ...
      if ($start >= $subcamp['start']) {
          ...
          $orderBy = " order by day_offset asc";
      } else {
          $summand = "+ ".($subcamp['start'] - $start);
          $orderBy = " order by day_offset desc";
      }
        
      $query = "UPDATE day SET day_offset=day_offset ".$summand." WHERE day.subcamp_id=".$subcamp['id'].$orderBy;
      mysqli_query($GLOBALS["___mysqli_ston"], $query);
