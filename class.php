 ...
 */
	...
	class page{
		...
		function addCssConfig($config){
			foreach($config as $file => $type){
				if($type == "app"){$this->addCssFile($GLOBALS['public_app_dir']."/".$this->app."/css/".$file);	}
				if($type == "global"){$this->addCssFile($GLOBALS['public_global_dir']."/css/".$file);}
				if($type == "module"){$this->addCssFile($GLOBALS['public_module_dir']."/css/".$file);}
			}
		}
		
		function addJsConfig($config){
			foreach($config as $file => $type){
				if($type == "app"){$this->addJsFile($GLOBALS['public_app_dir']."/".$this->app."/js/".$file);	}
				if($type == "global"){$this->addJsFile($GLOBALS['public_global_dir']."/js/".$file);}
				if($type == "module"){$this->addJsFile($GLOBALS['public_module_dir']."/js/".$file);}
			}
		}
		...
		function loadFileDependency($file){
			...
			if(trim(fgets($fh)) != "/** eCampConfig"){return;}
	...
	class camp{
		...
		function category($id){return $this->check( "SELECT id FROM category WHERE id = $id AND camp_id = $this->id" );}
