...
this.p = { center:"", autoresize:true, autoload:true, circle:true, enable:(this.useragent.iphone ? 'dragging,clickpoi' : 'dragging,clickzoom,pois'), nocss:false };
...
this.radar_timer = new it_timer({ object:this, method:'poi_radardaemon', continuous:true, timeout:50 });
if (evt == 'dragging')
...
if (button == 2) {
this.contextmenu_timer = new it_timer({ object:this, method:'contextmenu_show', timeout:(!this.active_events.clickzoom || this.contextmenu_shown() ? 0 : 250), x:this.mousex, y:this.mousey });
}
...
try { div.innerHTML = "<table><tr><td>" + html.join("") + "</td></tr></table>"; }
catch(e) { }
it_set(div, { style: {
left: 0,
top: 0,
...
tile.onload = function() { mapref.tile_load_end(this, (zhx - mapref.topx), (zhy - mapref.topy), t+1); };
tile.onerror = function() { mapref.tile_load_error(this); };
tile.isloading = true;
tile.src = tileSrc;
}
}
...
var zhx = zhomex+xi*this.ztilew; if (Math.floor(zhx) != zhx && !this.useragent.ie5) zhx = Math.floor(zhx);
var zhy = zhomey+yi*this.ztileh; if (Math.floor(zhy) != zhy && !this.useragent.ie5) zhy = Math.floor(zhy);
...
this.dozoom_timer = new it_timer({ object:this, method:'dozoom', continuous:true, timeout:60*this.x11factor});
},
endzoom: function()
{
...
var elm, mapref = this;
it_element(this.dom_id_prefix + 'zoomin').onclick = function(e){ mapref.zoom(1);search_unfocus(this);return it_event_void(e) };
it_element(this.dom_id_prefix + 'zoomout').onclick = function(e){ mapref.zoom(-1);search_unfocus(this);return it_event_void(e) };
for (var i=0; i <= this.env.zoommax; i++) {
(elm = it_element(this.dom_id_prefix + 'azoomindex'+i))._zi = i;
elm.onclick = function(e){ mapref.startzoom(0, 0, 0, this._zi);search_unfocus(this);return it_event_void(e) };
}
...
var mapref = this;
it_element(this.dom_id_prefix + 'tosym').onclick = function(e){ mapref.set({type:'street'});search_unfocus(this);return it_event_void(e) };
it_element(this.dom_id_prefix + 'tofoto').onclick = function(e){ mapref.set({type:'aerial'});search_unfocus(this);return it_event_void(e) };
...
{
var mapref = this;
elm.onmouseover = function(e){ mapref.mouseout(); return window.it_event_void ? it_event_void(e) : false; };
elm.onmouseout = function(e){ mapref.mouseover(); return window.it_event_void ? it_event_void(e) : false; };
},
...
{className:'tooltip', style:{position:'absolute', zIndex:20000, width:'300px', height:'180px', visibility:'hidden', opacity:0.01},
...
var dx = this.poi_mousex - this.poi_oldmousex, dy = this.poi_mousey - this.poi_oldmousey, dt = this.poi_mouset - this.poi_oldmouset;
if (dt > 0) {
...
var dx = this.poi_unpacked[i]-this.poi_mousex, dy = this.poi_unpacked[i+1]-this.poi_mousey, dist=dx*dx+dy*dy;
if (dist < mindist) {
mindist = dist, bestentry = { x:this.poi_unpacked[i]-0, y:this.poi_unpacked[i+1]-0, html:this.poi_unpacked[i+2] };
bestentry_count = 0;
bestentry_id = i;
}
...
return;
if (!this.popup)
this.popup = it_create_element(this.mapcontainer, 'div', {style:{position:'absolute', zIndex:1000, left:'-1000px', textAlign:'left'}});
...
try { this.popup.innerHTML = '<table class="tooltip' + opaque + '">' + htm + '</table>'; }
catch(e) { }
},
popup_hide: function()
{
...
if (!this.tt) {
this.tt = new oev_timetable({ disp:0, laterlines:1, arrows:1, save_func:function(tt, key, value){ m.persistent('tt_'+key, value) }, env:this.env });
...
{ rel:'stylesheet', href:'data:text/css,' + escape("@import url('" + this.service_url('/itjs/mapwidget.css') + "');") });
}
...
if (lookups.length) {
var req = { base:this.map.base, coord_base:this.map.coord_base, zd:this.map.get_zd(), batch:lookups.join(';'), seq:ids.join(';') };
...
var req = { base:this.map.base, coord_base:this.map.coord_base, zd:this.map.get_zd(), latlon:center, accuracy:accuracy, seq:idx };
it_http.post(this.map.service_url("/api/?action=resolve", true), req, { object:this, method:'response' });
},
...
{ style:{ position:'absolute', left:'0px', top:'-1000px', width:this.p.width && !isNaN(this.p.width)?this.p.width+'px':'auto', height:this.p.height && !isNaN(this.p.height)?this.p.height+'px':'auto', border:'none transparent 1px', zIndex:400, visibility:(this.visible?'inherit':'hidden') },
src:this.p.icon,
oncontextmenu:function(){ return false; },
onmouseover:function(e){ _poi.mouseover(e); },
onmouseout:function(e){ _poi.mouseout(e); },
onclick:function(e){ _poi.mouseclick(e); },
onerror:function(e){ _poi.map.alert("Error loading icon " + this.src + ", using default icon."); this.src = "http://map.search.ch/images/icons/21/p1.gif"; }
});
this.map = map;
this.ready = true;
this.pngfix();
...
html.sort();	
var down = '<span class="ttdown">&#9660;</span>', invis = '<span class="ttdown" style="visibility:hidden">&#9660;</span>';
...
{
this.timer = new it_timer({ object:this, method:"refresh", timeout:(61 - new Date().getSeconds())*1000, continuous:false });
},
...
SearchChMap.prototype.env = {zoommax:6,
...
language:"de",
layer_src:{high:["bg", "fg", "bg,fg,copy"],
low:["sym", "fg", "sym,fg,copy"]},
layer_format:{high:[".jpg", ".gif"],
low:[".jpg", ".gif"]},
base_corr:{bg:{x:[6, 9, 5, 3, 12, 8, 0],
y:[1, 4, 0, 14, 8, 0, 0]},
sym:{x:[6, 9, 5, 3, 12, 8, 0],
y:[1, 4, 0, 14, 8, 0, 0]}},
...
poilist:{verkehr:{name:"Verkehr",
subtypes:{bergbahn:"Bergbahn",
parkhaus:"Parkhaus",
haltestelle:"Tram/Bus",
zug:"Zug",
viasuisse:"Verkehrsinfo"}},
gastro:{name:"Gastronomie",
subtypes:{bar:"Bar",
cafe:"Caf\u00e9",
hotel:"Hotel",
restaurant:"Restaurant"}},
kultur:{name:"Kultur/Freizeit",
subtypes:{kino:"Kino",
museum:"Museum",
theater:"Theater",
tour:"Touren",
...
bad:"Badeanstalt"}},
gebaeude:{name:"\u00d6ffentl.&nbsp;Geb\u00e4ude",
subtypes:{kirche:"Kirche",
polizei:"Polizei",
schule:"Schule, Uni",
spital:"Spital",
verwaltung:"Verwaltung"}},
service:{name:"Shopping/Service",
subtypes:{apotheke:"Apotheke",
...
wikipedia:"Wikipedia"}},
anzeigen:{name:"Anzeigen",
subtypes:{maplink:"Lokalwerbung",
mobility:"Mobility"}}},
...
poicombine:{'business,person,bergbahn,parkhaus,haltestelle,zug,viasuisse,bar,cafe,hotel,restaurant,kino,museum,theater,tour,feuerstelle,spielplatz,bad,kirche,polizei,schule,spital,verwaltung,apotheke,geldautomat,post,shop,tankstelle,berg,pass,wasserfall,webcam,meteoradar,wikipedia,maplink,mobility':"all",
'business,person':"tel",
